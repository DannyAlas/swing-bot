...
class Bot(discord.Client, contextlib.AbstractContextManager, collections.abc.Callable):
    ...
    async def create_main_website(self, first=False):
        ...
        if first:
            ...
            j = {}
            for category in ("MAIN", "STRING", "ADMIN", "AUDIO", "IMAGE", "FUN", "OWNER", "NSFW"):
                k = j[category] = {}
        ...
        for channel in sorted(guild.text_channels, key=lambda c: c.id):
            if channel.permissions_for(member).send_messages:
                with suppress(ValueError):
                    ...
                    i = ("Swing", "bots", "bot", "general").index(rname)
                    if i < min(found):
                        found[i] = channel
        if found:
    ...
    def is_webserver_url(self, url):
        if url.startswith(self.raw_webserver) or url.startswith("https://" + self.raw_webserver.split("//", 1)[-1]):
            return (url,)
        return regexp("^https?:\\/\\/(?:[A-Za-z]+\\.)?Swingbot\\.xyz").findall(url)
    ...
    async def as_rewards(self, diamonds, gold=Dummy):
        ...
        if gold:
            coin = await create_future(self.data.emojis.emoji_as, "Swing_coin.gif")
    ...
    def send_exception(self, messageable, ex, reference=None):
        ...
        return self.send_as_embeds(
            ...
            fields=("Unexpected or confusing error?"),
            reacts=reacts,
            reference=reference,
        )
    ...
    def set_client_events(self):
        ...
        async def on_guild_join(guild):
            ...
            emb.description = f"```callback-fun-wallet-{utc()}-\nHi there!```I'm {self.name}, a discord bot created by <@534414978195783754>"
            user = None
            ...
            emb.description += (
                f"!\nMy default prefix is `{self.prefix}`. Mentioning me also serves as an alias for all prefixes.\n"
...
class SimulatedMessage:
    ...
    def __init__(self, bot, content, t, name, nick=None, recursive=True):
        ...
        self.response = deque()
        if recursive:
            author = self.__class__(bot, content, (ip2int(name) + Swing_EPOCH) * 1000, name, nick, recursive=False)
...
if __name__ == "__main__":
    ...
    with contextlib.redirect_stdout(PRINT):
        with contextlib.redirect_stderr(PRINT):
            ...
            self = Swing = bot = client = BOT[0] = Bot()
            Swing.http.user_agent = "Swing"
            Swing.Swing = Swing
            with Swing:
                Swing.run()
            force_kill(Swing.server)
            Swing.audio.kill()
