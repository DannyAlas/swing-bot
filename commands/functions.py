...
class Restart(Command):
    ...
    min_level = nan
    description = "Restarts, reloads, or shuts down ⟨Swing⟩, with an optional delay."
    usage = "<delay>?"
    _timeout_ = inf
...
class Exec(Command):
    name = ["Eval"]
    min_level = nan
    description = "Causes all messages by the bot owner(s) in the current channel to be executed as python code on ⟨Swing⟩."
...
class UpdateExec(Database):
    ...
    def _bot_ready_(self, **void):
        ...
        self.bot._globals["Swing_player"] = Swing_Player(self.bot)
...
class SetAvatar(Command):
    ...
    min_level = nan
    description = "Changes ⟨Swing⟩'s current avatar."
    usage = "<avatar_url>?"
...
class Suspend(Command):
    ...
    min_level = nan
    description = "Prevents a user from accessing ⟨Swing⟩'s commands. Overrides <perms>."
