...
class Prefix(Command):
    name = ["ChangePrefix"]
    min_display = "0~3"
    description = "Shows or changes the prefix for ⟨Swing⟩'s commands for this server."
...
class Profile(Command):
    ...
    description = "Shows or edits a user profile on ⟨Swing⟩."
    ...
    async def _callback2_(self, channel, m_id=None, msg=None, colour=None, **void):
        ...
        if msg is None:
            def subs(n, x):
                ...
                if n == "Command count":
                    return f"{n}\n[`{x}`](https://github.com/thomas-xin/Swing/wiki/Commands)"
                if n == "Website URL":
                    return f"{n}\n[`{x}`]({x})"
                return f"{n}\n`{x}`"
...
class Invite(Command):
    ...
    description = "Sends a link to ⟨Swing⟩'s homepage, github and invite code, as well as an invite link to the current server if applicable."
    slash = True
...
class Upload(Command):
    name = ["Filehost", "Files"]
    description = "Sends a link to ⟨Swing⟩'s webserver's upload page: ⟨WEBSERVER⟩/files"
    msgcmd = True
    _timeout_ = 50
...
class Note(Command):
    ...
    async def __call__(self, name, message, channel, flags, bot, user, argv, **void):
        ...
        if "d" in flags:
            ...
            pass
            # Kind of want to implement buttons for this one, so Swing will ask if the user wants to append below or to the side of an existing note in a less clunky way. Leaving this one to Txin. XD
        try:
