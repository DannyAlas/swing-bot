...
class React(Command):
    ...
    name = ["AutoReact"]
    min_level = 2
    description = "Causes ⟨Swing⟩ to automatically assign a reaction to messages containing the substring."
...
class Dogpile(Command):
    server_only = True
    min_level = 2
    description = "Causes ⟨Swing⟩ to automatically imitate users when 3+ of the same messages are posted in a row. Grants XP and gold when triggered."
...
class Rickroll(Command):
    ...
    async def __call__(self, bot, args, message, **void):
        ...
        if not args:
            return "https://Swingbot.xyz/view/!247184721262411780"
        if len(args) < 2:
